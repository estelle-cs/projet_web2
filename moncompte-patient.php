<?php
	...
	if(isset($_POST['oldmdp'])){
			$req = "SELECT * FROM patient WHERE id=".$_SESSION['id'].";";
			$ORes = $Bdd->query($req);
			...
			if(md5($_POST['oldmdp'])==$mdp_bdd){
				if(isset($_POST['newmdp1'])||isset($_POST['newmdp2'])){
							if($_POST['newmdp1']!=$_POST['newmdp2']){
								...
								//Changer mdp
								$reqChangePass = "UPDATE patient SET mdp='$newmdp' WHERE id='".$_SESSION["id"]."'";
				...
				if(isset($_POST["email"])){
					$email=$_POST["email"];
					$req="UPDATE patient SET email='$email' WHERE id='".$_SESSION["id"]."'";
					$Ores = $Bdd->query($req);
				}
				if(isset($_POST["name"])){
					$name=$_POST["name"];
					$req="UPDATE patient SET name='$name' WHERE id='".$_SESSION["id"]."'";
					$Ores = $Bdd->query($req);
				}
				if(isset($_POST["surname"])){
					$surname=$_POST["surname"];
					$req="UPDATE patient SET surname='$surname' WHERE id='".$_SESSION["id"]."'";
					$Ores = $Bdd->query($req);
				}
				if(isset($_POST["tel"])){
					$tel=$_POST["tel"];
					$req="UPDATE patient SET tel='$tel' WHERE id='".$_SESSION["id"]."'";
					$Ores = $Bdd->query($req);
				}
				if(isset($_POST["numSecu"])){
					$numSecu=$_POST["numSecu"];
					$req="UPDATE patient SET numSecu='$numSecu' WHERE id='".$_SESSION["id"]."'";
					$Ores = $Bdd->query($req);
				}
			}else{
...
?>
<a href="javascript:alert('Svp rediriger vers la prise de rendez-vous du patient!')" class="primary">Prendre rendez-vous</a>
...
<input required name="name" type="text" placeholder="Nom" value="<?php
    ...
    ?>">
	<input required name="numSecu" type="text" placeholder="Numéro de sécurité sociale" value="<?php
        $req = "SELECT * FROM patient WHERE id=".$_SESSION['id'].";";
        $ORes = $Bdd->query($req);
        while($users = $ORes->fetch())
        {
