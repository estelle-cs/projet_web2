<?php
	...
	try {
		...
		if(isset($_POST['oldmdp'])){
			$req = "SELECT * FROM secretariat WHERE id=".$_SESSION['id'].";";
			$ORes = $Bdd->query($req);
			...
			if(md5($_POST['oldmdp'])==$mdp_bdd){
				if(isset($_POST['newmdp1'])||isset($_POST['newmdp2'])){
							if($_POST['newmdp1']!=$_POST['newmdp2']){
								...
								//Changer mdp
								$reqChangePass = "UPDATE secretariat SET mdp='$newmdp' WHERE id='".$_SESSION["id"]."'";
				...
				if(isset($_POST["email"])){
					$email=$_POST["email"];
					$req="UPDATE secretariat SET email='$email' WHERE id='".$_SESSION["id"]."'";
					$Ores = $Bdd->query($req);
				}
				if(isset($_POST["name"])){
					$name=$_POST["name"];
					$req="UPDATE secretariat SET name='$name' WHERE id='".$_SESSION["id"]."'";
					$Ores = $Bdd->query($req);
				}
				if(isset($_POST["surname"])){
					$surname=$_POST["surname"];
					$req="UPDATE secretariat SET surname='$surname' WHERE id='".$_SESSION["id"]."'";
					$Ores = $Bdd->query($req);
				}
				if(isset($_POST["tel"])){
					$tel=$_POST["tel"];
					$req="UPDATE secretariat SET tel='$tel' WHERE id='".$_SESSION["id"]."'";
					$Ores = $Bdd->query($req);
				}
			}else{
...
?>
<a href="javascript:alert('Svp rediriger vers le calendrier du secretariat!')" class="primary">Gérer les rendez-vous</a>
